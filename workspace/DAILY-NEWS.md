...
各リクエスト間に `sleep 3`。対象: ETN, IONQ, MDB, NVDA, PLTR, TSLA, TSM, VRT（最大8リクエスト）
...
### 3i. 中間選挙・トランプ政策動向（戦略モニタリング）

`q=Trump+Iran+midterm+election+2026+military+market&count=3&freshness=pd`

対象イベント（以下のいずれかが検出されたら🔴要注意に昇格）:
- ホルムズ海峡の封鎖継続 / 解除
- トランプの「勝利宣言」「作戦終了」発言
- 原油価格 $85超 / $90超
- S&P500が前週比 -5%超
...
`q=quantum+computing+IONQ&count=3&freshness=pw`
...
labels = ['NVDA','SPY','TSLA','TSM','PLTR','URA','MDB','VRT','ETN','1326.T','1542.T','IONQ']
data = [1.2, -0.3, 2.5, -1.1, 3.2, -0.5, -2.1, 1.8, -0.7, 0.8, 1.5, -4.2]  # Step 1の実データで置き換え
...
### 戦略見解（3フェーズ買い増しプラン）
`memory/2026-03-04.md` の「3フェーズ買い増しプラン」を参照し、以下を判定する:

**フェーズステータス判定:**
- フェーズ1（3月）: 実行済みかどうか（stock-history.jsonのPLTR/NVDA/SPY株数で判断）
- フェーズ2トリガー監視:
  - SPY現在値 vs $646（-5%水準）
  - 原油価格（3i.のニュースから推定）vs $85
  - PLTR現在値 vs $135
  - NVDA現在値 vs $165
  - 各トリガーの状態: 未達（緑）/ 接近=10%以内（橙）/ 発動（赤）
- フェーズ3（7〜8月）: ロック中

**本日の戦略コメント生成ルール:**
1. イラン紛争・ホルムズ海峡の最新動向（3i.の検索結果）を踏まえる
2. 中間選挙まで何ヶ月か（現在日付から11月第1火曜日までの残日数）を明示
3. フェーズ2トリガーの接近・発動があれば警告
4. 「勝利宣言」「作戦終了」シグナルを検出したらフェーズ3前倒しを示唆
5. 2〜4行の簡潔なコメントで締める
	...
	<table style="width:100%;border-collapse:collapse">
		<tr>
			...
			<td style="padding-left:12px;vertical-align:middle">
				...
				<p style="margin:2px 0 0;font-size:14px;color:#888">{YYYY}年{MM}月{DD}日（{曜日}）</p>
			</td>
		</tr>
	</table>
</div>
...
<div style="background:#f0f2f8;padding:20px 24px">
	<div style="font-size:15px;font-weight:700;margin-bottom:14px;color:#1a1a2e">📌 あなたのポートフォリオへの影響サマリー</div>
...
<div style="background:#ffffff;padding:16px 24px">
	<div style="font-size:13px;font-weight:600;margin-bottom:10px;color:#888;letter-spacing:0.5px">KEY DRIVERS</div>
	<table style="width:100%;border-collapse:collapse;font-size:14px">
		...
		<tr>
			...
			<td style="padding:6px 0;padding-left:12px;font-size:12px;color:#888">{短い説明}</td>
		</tr>
		<!-- 6行繰り返し -->
	</table>
</div>
...
<div style="background:#f8f6ff;padding:16px 24px">
	<div style="font-size:13px;font-weight:600;margin-bottom:8px;color:#7c3aed;letter-spacing:0.5px">💡 今週のアクション</div>
	<div style="font-size:15px;line-height:1.8;color:#333">
...
<div style="padding:24px 24px 0">
	<div style="font-size:13px;font-weight:600;color:#888;margin-bottom:6px;letter-spacing:0.3px">週間パフォーマンス（保有額順）</div>
</div>
...
<div style="background:#f5f5ff;padding:20px 24px 24px;margin-bottom:2px">
	<h2 style="font-size:17px;margin:0 0 16px 0;color:#444">📅 今週の注目日</h2>
	...
	<div style="border-left:3px solid {色};padding:12px 16px;margin-bottom:12px;background:white;border-radius:0 10px 10px 0">
		<div style="font-size:15px;font-weight:700;color:{色}">{M/D}（{曜}）— {重要度ラベル}</div>
		<div style="font-size:15px;color:#555;margin-top:4px;line-height:1.6">
			{説明。保有銘柄への影響を太字で。}
		</div>
	</div>
	<!-- 日付カードを繰り返し（経済指標 + 決算日） -->
...
<!-- ===== 🔴 要注意 ===== -->
<div style="padding:24px 24px 0">
	<h2 style="font-size:17px;border-left:4px solid #e74c3c;padding-left:12px;margin:0 0 16px 0;color:#e74c3c">🔴 要注意</h2>
</div>
<!-- ニュース項目を繰り返し -->
<div style="background:#fef5f5;padding:16px 24px;margin-bottom:2px">
	...
	<div style="clear:both;font-size:15px;color:#444;line-height:1.7;border-left:3px solid #e74c3c;padding-left:12px;margin-top:8px">
...
<!-- ===== 🟡 注視 ===== -->
<div style="padding:24px 24px 0">
	<h2 style="font-size:17px;border-left:4px solid #f39c12;padding-left:12px;margin:0 0 16px 0;color:#e67e22">🟡 注視</h2>
</div>
...
<!-- ===== 🟢 参考 ===== -->
<div style="padding:24px 24px 0">
	<h2 style="font-size:17px;border-left:4px solid #27ae60;padding-left:12px;margin:0 0 16px 0;color:#27ae60">🟢 参考</h2>
</div>
...
<!-- ===== STRATEGY OUTLOOK ===== -->
<div style="background:#fff8e1;padding:20px 24px;margin-bottom:2px">
	<div style="font-size:15px;font-weight:700;margin-bottom:14px;color:#b45309">🗺️ 戦略見解（3フェーズ買い増しプラン）</div>

	<!-- フェーズステータスバー -->
	<table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:14px">
		<tr>
			<td style="padding:6px 10px;background:#4ade80;color:#fff;font-weight:700;border-radius:6px 0 0 6px;text-align:center">✅ フェーズ1<br><span style="font-size:11px;font-weight:400">3月 ¥35万</span></td>
			<td style="padding:6px 10px;background:{フェーズ2色};color:#fff;font-weight:700;text-align:center">⏳ フェーズ2<br><span style="font-size:11px;font-weight:400">4〜5月 ¥25万</span></td>
			<td style="padding:6px 10px;background:#94a3b8;color:#fff;font-weight:700;border-radius:0 6px 6px 0;text-align:center">🔒 フェーズ3<br><span style="font-size:11px;font-weight:400">7〜8月 ¥50万</span></td>
		</tr>
	</table>
	<!-- フェーズ2色: 待機中=#94a3b8 / トリガー接近=#f59e0b / 実行中=#3b82f6 -->

	<!-- トリガー監視 -->
	<div style="font-size:13px;font-weight:600;color:#b45309;margin-bottom:8px">📡 フェーズ2トリガー監視</div>
	<table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:14px">
		<tr style="background:#fef3c7">
			<td style="padding:5px 8px;color:#555">S&P500 $646以下</td>
			<td style="padding:5px 8px;text-align:right;font-weight:700;color:{SPY判定色}">{SPY現在値} — {未達/接近/発動}</td>
		</tr>
		<tr>
			<td style="padding:5px 8px;color:#555">原油 $85超</td>
			<td style="padding:5px 8px;text-align:right;font-weight:700;color:{原油判定色}">{原油現在値} — {未達/接近/発動}</td>
		</tr>
		<tr style="background:#fef3c7">
			<td style="padding:5px 8px;color:#555">PLTR $135以下</td>
			<td style="padding:5px 8px;text-align:right;font-weight:700;color:{PLTR判定色}">{PLTR現在値} — {未達/接近/発動}</td>
		</tr>
		<tr>
			<td style="padding:5px 8px;color:#555">NVDA $165以下</td>
			<td style="padding:5px 8px;text-align:right;font-weight:700;color:{NVDA判定色}">{NVDA現在値} — {未達/接近/発動}</td>
		</tr>
	</table>
	<!-- 判定色: 未達=#27ae60 / 接近（10%以内）=#e67e22 / 発動=#e74c3c -->

	<!-- 今日の戦略コメント -->
	<div style="font-size:13px;font-weight:600;color:#b45309;margin-bottom:6px">💬 本日の戦略見解</div>
	<div style="font-size:14px;line-height:1.8;color:#444;border-left:3px solid #f59e0b;padding-left:12px">
		{世界情勢を踏まえた戦略コメント。以下の観点で2〜4行で記述:
		- イラン紛争・ホルムズ海峡の最新状況と3フェーズプランへの影響
		- 中間選挙タイムライン上の現在地（「夏の底」まで何ヶ月か）
		- PLTR・NVDA・SPYの動向と各フェーズの実行判断
		- 「勝利宣言」シグナルの有無}
	</div>
</div>
...
<!-- ===== SECTOR OUTLOOK ===== -->
<div style="padding:24px 24px 0">
	<h2 style="font-size:17px;color:#444;margin:0 0 16px 0">📊 セクター別展望</h2>
</div>
<!-- セクターごとに異なる背景色 -->
<div style="background:#f8f8ff;padding:16px 24px;margin-bottom:2px">
	<div style="font-size:16px;font-weight:600;margin-bottom:6px">🤖 AI・半導体 <span style="font-size:13px;color:#888;font-weight:400">NVDA, TSM, PLTR</span></div>
	<div style="font-size:15px;color:#555;line-height:1.7">{展望}</div>
</div>
<div style="background:#fffff0;padding:16px 24px;margin-bottom:2px">
	<div style="font-size:16px;font-weight:600;margin-bottom:6px">🥇 貴金属 <span style="font-size:13px;color:#888;font-weight:400">1326.T, 1542.T</span></div>
	<div style="font-size:15px;color:#555;line-height:1.7">{展望}</div>
</div>
<div style="background:#f0fff0;padding:16px 24px;margin-bottom:2px">
	<div style="font-size:16px;font-weight:600;margin-bottom:6px">☢️ ウラン <span style="font-size:13px;color:#888;font-weight:400">URA</span></div>
	<div style="font-size:15px;color:#555;line-height:1.7">{展望}</div>
</div>
<div style="background:#f5f0ff;padding:16px 24px;margin-bottom:2px">
	<div style="font-size:16px;font-weight:600;margin-bottom:6px">🖥️ DCインフラ <span style="font-size:13px;color:#888;font-weight:400">VRT, ETN</span></div>
	<div style="font-size:15px;color:#555;line-height:1.7">{展望}</div>
</div>
<div style="background:#f0f8ff;padding:16px 24px">
	<div style="font-size:16px;font-weight:600;margin-bottom:6px">⚛️ 量子コンピューティング <span style="font-size:13px;color:#888;font-weight:400">IONQ</span></div>
	<div style="font-size:15px;color:#555;line-height:1.7">{展望}</div>
</div>
...
- 1回の実行: 約22リクエスト（ニュース13 + 決算8 + 戦略モニタリング1）
- 月間（平日22日）: 約484リクエスト
